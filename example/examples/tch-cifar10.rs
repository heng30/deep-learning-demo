...
use tch::{
    nn::{self, ModuleT, Optimizer, OptimizerConfig, Sequential, VarStore},
    Device, IndexOp, Kind, Tensor,
};

const BATCH_SIZE: i64 = 32;
const TRAIN_EPOCH: u32 = 20;
...
async fn main() -> Result<()> {
    ...
    println!("\n===================START TRAIN====================");
    println!("Train dataset loading ...");
    let mut net = Cifar10Net::new(BATCH_SIZE, device).await?;
    net.train(TRAIN_EPOCH, MODEL_PATH)?;

    println!("\n===================START TEST====================");
    Cifar10Net::test(MODEL_PATH, BATCH_SIZE, device).await?;
...
impl Cifar10Net {
    ...
    pub fn forward(&self, xs: &Tensor, is_train: bool) -> Tensor {
    ...
    pub fn train(&mut self, epoch: u32, save_path: &str) -> Result<()> {
        // label的值为0-9中的一个
        ...
        for epoch in 1..=epoch {
            ...
            //  打乱数据
            let permutation = Tensor::randperm(train_labels_size[0], (Kind::Int64, self.device));
            let train_images = train_images.index_select(0, &permutation);
            let train_labels = train_labels.index_select(0, &permutation);
            ...
            for batch_index in 0..(train_images_size[0] / self.batch_size) {
                ...
                let output = self.forward(train_dataset, true);
    ...
    pub async fn test(model_path: &str, batch_size: i64, device: Device) -> Result<()> {
        ...
        for batch_index in 0..(test_images_size[0] / batch_size) {
            ...
            // 运行模型
            let output = net.forward(&test_dataset, false);
            ...
            println!(
                "Batch_index: {:4}  Correct: {:.3}",
                batch_index,
                batch_correct as f64 / batch_size as f64
            );
        }

        println!(
            "Toatl test Correct: {:.3}",
