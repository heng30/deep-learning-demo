...
fn main() -> Result<()> {
    ...
    let embedding_dim = 4; // 为了观察方便使用4。在具体训练中应该取一个比较大的值：例如128

    // 3. 构建嵌入层网络
    let net = Net::new(vocab_size as i64, embedding_dim, BATCH_SIZE, device);

    println!("\n============= Embedding Vector=============");
    for word in tokens.iter() {
        let idx = vocab.get_id(word) as i64;
        let xs = Tensor::from(idx);
        let output = net.forward_t(&xs, true);
...
impl Net {
    pub fn new(vocab_size: i64, embedding_dim: i64, batch_size: i64, device: Device) -> Self {
